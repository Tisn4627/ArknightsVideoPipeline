...
:class:`~ArknightsVideoRecognition.formation.FormationOper` 的 ``avatar``
做 ``cv2.matchTemplate``（``TM_CCOEFF_NORMED``），取最高分且超阈值的定为
该干员名。一个干员名可能匹配多个场上框（同一干员多次部署），允许重复。

``screen_pos_to_tile`` 用 :func:`~ArknightsVideoRecognition.tile.get_all_tile_positions`
...
class AvatarMatcher:
    ...
    def match(
        ...
        ----------
        detections:
            :class:`~ArknightsVideoRecognition.battle.detector.Detection` 列表。
        formation_opers:
            :class:`~ArknightsVideoRecognition.formation.FormationOper` 列表。
        frame:
...
def screen_pos_to_tile(
    ...
    screen_pos:
        待反查的屏幕坐标 ``(x, y)``。
    level:
        关卡 dict（来自 :func:`~ArknightsVideoRecognition.tile.find_level`）。
    screen_size:
